 ...
 * Add attachment
 */
exports.addAttachment = async (data) => {
  const {
    maintenance_id,
    filename,
    file_path,
  ...
  const result = await db.query(
    ...
    (maintenance_id, filename, file_path, url, uploaded_by)
    VALUES ($1,$2,$3,$4,$5)
    RETURNING *
    `,
    [maintenance_id, filename, file_path, url, uploaded_by]
...
/**
 * Get attachments for maintenance
 */
exports.getAttachments = async (maintenance_id) => {
  const result = await db.query(
    `
    SELECT *
