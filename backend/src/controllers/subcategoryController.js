...
exports.create = async (req, res, next) => {
  try {
    ...
    if (!subcategory_name || !category_id) {
      return res
        .status(400)
        .json({ error: "Missing required subcategory name or category." });
    }

    // Check if subcategory already exists for the given category
    const existing = await subcategoryModel.getByNameAndCategory(
      subcategory_name,
      category_id
    );
    if (existing) {
      return res
        .status(409)
        .json({
          error: "Subcategory already exists in the specified category.",
        });
    }
...
exports.updateDesc = async (req, res, next) => {
  try {
    ...
    if (!description) {
      return res
        .status(400)
        .json({ error: "Missing required subcategory description." });
    }

    const updated = await subcategoryModel.updateSubcategoryDescription(
      id,
      description
    );
    if (!updated) {
...
exports.list = async (req, res, next) => {
  try {
    const category_id = req.params.cat;
    const subcategories = await subcategoryModel.listAllSubcategories(category_id);
...
exports.listByCategory = async (req, res, next) => {
  try {
    ...
    for (const row of subcategories) {
      grouped[row.category_id] = row.subcategories;
    }
